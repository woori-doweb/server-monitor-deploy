...
(function() {
    ...
    function loadDatabases() {
        ...
        if (!container) return;
        // 보조 서버: DB 미설치. PostgreSQL 도입 시 신규 모듈 작성 (Phase 3 후보)
        container.innerHTML = '<div class="result-box result-warn">' +
            '<strong>DB 미설치</strong> — 보조 서버에는 현재 데이터베이스가 설치되어 있지 않습니다.<br>' +
            'PostgreSQL 도입 결정 시 별도 ADR 작성 후 모듈 추가 예정 (Phase 3).' +
            '</div>';
        return;
        // 아래 코드는 비활성 (PostgreSQL 모듈로 교체 예정)
