...
(function() {
    ...
    // workstation (192.168.1.100 워크스테이션 — Ubuntu 22.04, Tailscale serve, ufw, Docker, no web server)
    SM.PROTECTED_VHOSTS = []; // 웹서버 없음. Tailscale serve가 path-based 라우팅
    SM.PROTECTED_ACCOUNTS = ["root", "monitor", "your-org", "yms", "yes1021", "sftp_trader"];
    SM.PROTECTED_SERVICES = ["ssh", "ufw", "tailscaled", "pm2-monitor", "lightdm"];
    SM.PROTECTED_PORTS = [19999, 3010, 9090, 22, 3001, 4077, 3363, 18790, 3389]; // Netdata, Uptime Kuma, Cockpit, SSH, trader-admin, auth API, shell, /monitor, xrdp
    SM.SYSTEM_DB_USERS = []; // DB 미설치
