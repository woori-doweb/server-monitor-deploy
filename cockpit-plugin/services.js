...
(function() {
    ...
    var NO_DISABLE = SM.PROTECTED_SERVICES || ["sshd", "firewalld"];
    var WARN_STOP = ["httpd", "mariadb", "sshd"];
    ...
    function loadServices() {
        ...
        function checkDone() {
            if (--pending > 0) return;
            addPm2Services(function(pm2svcs) {
                results = results.concat(pm2svcs);
    ...
    function controlService(name, action) {
        ...
        if (needsTyped) {
            ...
            // 2026-05-05 사고(httpd stop으로 모든 vhost 다운)의 재발 방지
            SM.typedConfirmDialog("Stop Critical Service",
                "Service: " + name + "\n\n" +
                "이 서비스를 stop하면 호스팅 중인 모든 사이트가 즉시 다운됩니다.\n" +
                "재시작 시 권한·SELinux 정책 정합성 문제로 시작 실패할 위험이 있어\n" +
                "운영 시간에는 신중히 결정하세요.",
                name)
