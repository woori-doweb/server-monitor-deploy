...
(function() {
    ...
    function loadTimerStatus() {
        ...
        if (!el) return;
        cockpit.spawn(["systemctl", "is-active", "certbot-renew.timer"])
            .then(function(out) {
        ...
        SM.confirmDialog("Renew SSL Certificate", "Certificate: " + certName + "\nThis will renew via certbot and reload Apache.")
            .then(function(ok) {
                ...
                SM.runActionChain("ssl-action-result", [
                    ...
                    {
                        label: "Apache 문법 검사 (httpd -t)",
                        required: true,
                        fn: function() { return cockpit.spawn(["httpd", "-t"], { superuser: "require" }); }
                    },
                    {
                        label: "Apache 리로드",
                        required: false,
                        fn: function() { return cockpit.spawn(["systemctl", "reload", "httpd"], { superuser: "require" }); }
                    }
                ], function(result) {
            ...
            .then(function(ok) {
                ...
                SM.runActionChain("ssl-action-result", [
                    ...
                    },
                    {
                        ...
                        fn: function() { return cockpit.spawn(["httpd", "-t"], { superuser: "require" }); },
                        pendingAction: {
                            label: "Apache 설정 수정 필요 (삭제된 인증서 참조)",
    ...
    function issueCert() {
        ...
        var method = ($("ssl-method") || {}).value || "apache";
        ...
        if (method === "apache") {
            steps = [
                {
                    label: "certbot --apache " + domains.join(", "),
                    required: true,
                    fn: function() {
                        var cmd = ["certbot", "--apache", "--non-interactive", "--agree-tos", "--email", email].concat(domainArgs);
                ...
                {
                    label: "Apache 리로드 (SSL 적용)",
                    required: false,
                    fn: function() { return cockpit.spawn(["systemctl", "reload", "httpd"], { superuser: "require" }); },
                    pendingAction: {
                        label: "Apache 리로드 (SSL 발급 적용)",
            ...
            steps = [
                ...
                {
                    label: "⚠ Apache VHost에 SSL 설정 수동 추가 필요",
                    required: false,
                    fn: function() {
                        return cockpit.spawn(["httpd", "-t"], { superuser: "require" });
                    },
                    pendingAction: {
                        label: "Apache VHost SSL 설정 + 리로드 필요",
    ...
    function updateDepPreview() {
        ...
        if (method === "apache") {
            steps = [
                { label: "certbot --apache -d {domain}", auto: true },
                { label: "Apache VHost에 SSL 지시어 자동 추가", auto: true },
                { label: "Apache 문법 검사 (httpd -t)", auto: true },
                { label: "Apache 리로드", auto: true, duration: "30-60초" }
            ];
        } else {
            steps = [
                { label: "certbot certonly --standalone -d {domain}", auto: true },
                { label: "Port 80 일시 점유 (인증 검증)", auto: true },
                { label: "Apache VHost SSL 설정 추가", auto: false },
                { label: "Apache 리로드", auto: false, duration: "30-60초" }
            ];
        }
