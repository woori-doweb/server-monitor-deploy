...
(function() {
    ...
    function loadCronJobs() {
        ...
        // User crontabs
        cockpit.spawn(["sh", "-c", "for u in root monitor app1 app3; do echo \"==$u==\"; crontab -l -u $u 2>/dev/null || echo '(none)'; done"], { superuser: "require" })
    ...
    function loadDiskUsage() {
        ...
        cockpit.spawn(["du", "-sh", "/home/app1", "/home/app3", "/home/monitor", "/var/log", "/tmp"], { superuser: "require" })
            .then(function(output) {
    ...
    function loadLargeFiles() {
        ...
        cockpit.spawn(["find", "/home", "-size", "+100M", "-type", "f", "-exec", "ls", "-lh", "{}", ";"], { superuser: "require" })
            .then(function(output) {
                if (!output.trim()) {
    ...
    function loadUpdates() {
        ...
        cockpit.spawn(["dnf", "check-update", "--quiet"], { superuser: "require" })
            .then(function(output) {
                if (!output.trim()) {
                    container.innerHTML = '<div class="muted">System is up to date</div>';
                    return;
                }
                var lines = output.trim().split("\n").filter(function(l) { return l.trim() && !l.match(/^\s/); });
                container.innerHTML = '<div class="result-box result-success">' + lines.length + ' updates available</div>' +
                    '<pre class="log-output" style="max-height:200px;overflow:auto;margin-top:0.5rem">' + SM.escapeHtml(output.trim()) + '</pre>';
            })
            .fail(function(ex) {
                if (ex.exit_status === 100) {
                    // exit 100 means updates available
                    var output = ex.message || "";
                    container.innerHTML = '<div class="result-box result-success">Updates available</div>' +
                        '<pre class="log-output" style="max-height:200px;overflow:auto;margin-top:0.5rem">' + SM.escapeHtml(output) + '</pre>';
                } else {
    ...
    function loadSecurityAudit() {
        ...
        cockpit.spawn(["journalctl", "-u", "sshd", "--grep=Failed", "--since=-24h", "--no-pager", "-o", "short-iso"], { superuser: "require" })
            .then(function(output) {
