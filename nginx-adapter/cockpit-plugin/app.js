...
(function() {
    ...
    function updateServices() {
        ...
        // Uptime Kuma — Docker 컨테이너 (시스템 docker.service)
        cockpit.spawn(["sh", "-c",
            "docker ps -f name=uptime-kuma --format '{{.Status}}' 2>/dev/null | head -1"])
            .then(function(out) {
                var state = out.trim().toLowerCase().indexOf("up ") === 0 ? "active" : "inactive";
                results.push({ label: "Uptime Kuma", name: "docker:uptime-kuma", state: state });
                pending--;
                if (pending <= 0) render();
            })
            .fail(function() {
                results.push({ label: "Uptime Kuma", name: "docker:uptime-kuma", state: "inactive" });
                pending--;
                if (pending <= 0) render();
            });
    }
